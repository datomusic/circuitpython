...
STATIC mp_obj_t audioio_wavefile_obj_set_stretch(mp_obj_t self_in, mp_obj_t stretch_factor) {
    audioio_wavefile_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);
    common_hal_audioio_wavefile_set_stretch(self, mp_obj_get_int(stretch_factor));
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_2(audioio_wavefile_set_stretch_obj, audioio_wavefile_obj_set_stretch);
...
static const mp_rom_map_elem_t audioio_wavefile_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_set_stretch), MP_ROM_PTR(&audioio_wavefile_set_stretch_obj) },
